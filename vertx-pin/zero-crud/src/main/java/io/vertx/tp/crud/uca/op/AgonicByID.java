    ...
    public Future<JsonObject> runAsync(final JsonObject input, final IxIn in) {
        ...
        return jooq.fetchOneAsync(input).compose(entity -> {
            if (Objects.isNull(entity)) {
                ...
                final JsonObject entityJson = Ux.toJson(entity, module.getPojo());
                return Ix.seekFn(in).apply(entityJson)
                        .compose(category -> Ux.future(category.copy().mergeIn(entityJson)));
            }
        });
    }
}
