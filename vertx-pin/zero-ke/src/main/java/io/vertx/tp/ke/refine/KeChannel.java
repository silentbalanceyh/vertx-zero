...
class KeChannel {
    ...
    static <T, O> Future<O> channel(final Class<T> clazz, final Supplier<O> supplier,
        ...
        if (Objects.isNull(channel)) {
            KeLog.warnChannel(KeChannel.class, "Channel {0} null", clazz.getName());
            return Ux.future(supplier.get());
        } else {
            KeLog.debugChannel(KeChannel.class, "Channel Async selected {0}, {1}",
    ...
    static <T, O> O channelSync(final Class<T> clazz, final Supplier<O> supplier,
        ...
        final T channel = Pocket.lookup(clazz);
        if (Objects.isNull(channel)) {
            KeLog.warnChannel(KeChannel.class, "Channel Sync {0} null", clazz.getName());
            return supplier.get();
        } else {
            KeLog.debugChannel(KeChannel.class, "Channel Sync selected {0}, {1}",
                channel.getClass().getName(), String.valueOf(channel.hashCode()));
            return executor.apply(channel);
        }
    }
    ...
    static <T, O> Future<O> channelAsync(final Class<T> clazz, final Supplier<Future<O>> supplier,
        ...
        final T channel = Pocket.lookup(clazz);
        if (Objects.isNull(channel)) {
            KeLog.warnChannel(KeChannel.class, "Channel Async {0} null", clazz.getName());
            return supplier.get();
        } else {
            KeLog.debugChannel(KeChannel.class, "Channel Async selected {0}, {1}",
                channel.getClass().getName(), String.valueOf(channel.hashCode()));
