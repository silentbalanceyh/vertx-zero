...
ALTER TABLE O_USER
    ADD UNIQUE (`CLIENT_SECRET`);

-- 生成临时授权码：/oauth/authorize
ALTER TABLE O_USER
    ADD INDEX IDXM_O_USER_OAUTH_AUTHORIZE_APP (`SCOPE`, `CLIENT_ID`, `CLIENT_SECRET`);
ALTER TABLE O_USER
    ADD INDEX IDXM_O_USER_OAUTH_AUTHORIZE_FULL (`SCOPE`, `CLIENT_ID`, `CLIENT_SECRET`, `LANGUAGE`);
