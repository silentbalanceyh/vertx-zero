...
public class FanService implements FanStub {
    @Override
    public Future<JsonObject> preAsync(final FBill bill, final FBillItem billItem, final FPreAuthorize authorize) {
        ...
        return Ux.Jooq.on(FBillDao.class).insertAsync(bill).compose(inserted -> {
            ...
            return Ux.thenCombine(futures).compose(nil -> Ux.futureJ(bill));
        });
    }
