...
(
    36
) COMMENT '「activityId」- 关联的操作ID',

    -- 是否变更记录
    `TYPE` VARCHAR
(
    64
) COMMENT '「type」- 字段变更类型：ADD | DELETE | UPDATE 三种',
    `STATUS` VARCHAR
(
    64
) COMMENT '「status」- 待确认变更状态：CONFIRMED | PENDING',

    `FIELD_NAME` VARCHAR
(
    255
) COMMENT '「fieldName」- 如果是变更记录则需要生成变更日志',
    `FIELD_ALIAS` VARCHAR
(
    255
) COMMENT '「fieldAlias」- 字段对应的别名',
    `FIELD_TYPE` VARCHAR
(
    255
) COMMENT '「fieldType」- 变更字段的数据类型，直接从模型定义中读取',
