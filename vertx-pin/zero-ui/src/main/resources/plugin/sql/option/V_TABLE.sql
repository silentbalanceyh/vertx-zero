...
(
    ...
    -- 行选择，主要用于 onRow
    `ROW_DOUBLE_CLICK` VARCHAR
(
    64
) COMMENT '「rowDoubleClick」- row.onDoubleClick - 双击事件名',
    `ROW_CLICK` VARCHAR
(
    64
) COMMENT '「rowClick」- row.onClick - 单击事件名',
    `ROW_CONTEXT_MENU` VARCHAR
(
    64
) COMMENT '「rowContextMenu」- row.onContextMenu - 右键菜单事件名',
    `ROW_MOUSE_ENTER` VARCHAR
(
    64
) COMMENT '「rowMouseEnter」- row.onMouseEnter - 鼠标左键事件名',
    `ROW_MOUSE_LEAVE` VARCHAR
(
    64
) COMMENT '「rowMouseLeave」- row.onMouseLeave - 鼠标移开事件名',
    -- Executor 专用 columns（第一列/或最后一列）
    `OP_TITLE` VARCHAR
(
    255
) COMMENT '「opTitle」- columns/[0]/title, 执行列标题',
    `OP_DATA_INDEX` VARCHAR
(
    255
) COMMENT '「opDataIndex」- columns/[0]/dataIndex, 执行列标题',
