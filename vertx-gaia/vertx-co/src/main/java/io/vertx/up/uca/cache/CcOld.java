 ...
 */
public interface CcOld<K, T> {

    static <K, T> CcOld<K, T> open() {
        /*
         ...
         */
        return new CcOldMemory<>();
    }


    ConcurrentMap<K, T> pick();
    ...
    T pick(Class<?> key, Supplier<T> supplier);

    // Data Part ( For Configuration )
    boolean pickIn(K key);
     ...
     * @return Cached reference
     */
    T fiber(Supplier<T> supplier, String root);
     ...
     * @return Cached reference
     */
    T fiber(Supplier<T> supplier, Class<?> clazz);
}
