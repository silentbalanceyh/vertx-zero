...
import io.vertx.up.plugin.git.GitClient;
import io.vertx.up.plugin.git.GitInfix;
...
public class NovaGit extends AbstractNova {
    ...
    @Override
    public Future<Boolean> configure(final ConcurrentMap<EmCloud.Runtime, KRepo> repoMap) {
        /*
        ...
        // kinect 客户端
        final KRepo zeroP = repoMap.get(EmCloud.Runtime.kinect);
     ...
     * 如果已存在该库则     git pull
     */
    private Future<Boolean> connect(final ConcurrentMap<EmCloud.Runtime, KRepo> repoMap) {
        // kidd / kzero 下载
        final Function<EmCloud.Runtime, Future<Boolean>> connectFn = (runtime) -> {
            final KRepo zeroP = repoMap.get(runtime);
            final GitClient zeroC = GitInfix.createClient(this.vertx, zeroP);
            return zeroC.connectAsync(true).compose(nil -> Ux.futureT());
        };
        final List<Future<Boolean>> futures = new ArrayList<>();
        futures.add(connectFn.apply(EmCloud.Runtime.kzero));
        futures.add(connectFn.apply(EmCloud.Runtime.kidd));
